...
```text
右键文件/文件夹 -> 服务 -> 右键助手 · 剪切（示例直达动作）
右键文件/文件夹 -> 服务 -> 右键助手…（完整动作面板）
```

直达区将已启用的收藏按收藏顺序放在前面，再用“剪切、拷贝完整路径、拷贝文件名、在系统终端中打开、获取 SHA256”等常用动作补齐；自动去重且最多显示 8 项。高风险动作不会进入直达区，依赖未安装外部应用的动作也不会生成。修改收藏或动作开关后，右键助手会按需刷新系统服务，不会后台轮询。
