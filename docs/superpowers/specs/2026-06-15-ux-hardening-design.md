...
| 模块 | 路径 | 职责 | 解决 |
| --- | --- | --- | --- |
| AppLog | `Sources/RightClickAssistant/Core/Logging/AppLog.swift` | 极薄 `os.Logger` 包装，按 `subsystem = guyue.RightClickAssistant` 切 category（`extension` / `host` / `action.<id>` / `storage`），对外只暴露 `info / debug / error` | C1 |
